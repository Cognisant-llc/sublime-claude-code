...
**設定**は `activity_panel` ブロック: `auto_open`、`group` と `min_width_chars`（ペインの位置と幅）、`window_hours`、`show_code`、`scope`（`"all"` = 稼働中セッション全部のプロジェクト／`"window"` = このウィンドウのフォルダ配下のみ）、`hide_projects`、`prune`（一覧にも監視にも含めないディレクトリ名）、`keep_days`、`fit_to_view`。`fit_to_view`（既定 on）ではツリーをペインの高さに合わせます: 全プロジェクトの行を必ず残し、どのセッションも最新 1 件を出してから 2 件目に進み、行が足りなければ古いプロジェクトから 1 行に畳みます（`▷`。クリックで「フォーカス」`◆` になり、そのプロジェクトだけ `max_files_per_session` まで展開、他は残りを分け合う）。ログは `~/.claude/logs/file-activity*.jsonl` の 2 本で、30 分ごとに圧縮し、年齢（`keep_days`・既定 7 日）と件数の両方で上限を掛けます。ファイル監視側のログには文書・メディアの記録だけを残します。1 プロジェクトで 10 秒に 100 件を超える変更（`git checkout` や rebase がツリーを書き換えた時）はセッションの作業ではなく「嵐」として捨て、件数はパネル先頭に `⚡` で出します。ファイル監視は現状 Windows のみ（他 OS では hook 記録のみ表示）。

### セッション別タブ（0.3.8）

Claude セッションが開いた・書いたタブにはそのセッションが記録され、ブラウザのタブグループのように**色で見分けて、まとめて扱える**ようになります。

- **色付きタブ**: タブを開いているセッションごとに 8 色（等間隔の鮮やかな色。`session_tabs.palette: "scheme"` でお使いのスキームのパレット色）を割り当て、そのセッションのタブを同じ色で塗ります（非選択時は薄く）。Sublime はタブを「そのタブ自身のカラースキームの背景色」で塗るため、プラグインが hidden スキーム 8 本（お使いのスキーム＋色付き背景）を `Packages/User/Claude Code IDE Sessions/` に書き出してセッションごとに割り当てます。そのタブのエディタ背景も同じ色味になります（強さは `session_tabs.tint`）。非選択タブに色を出すテーマ規則を Default / Default Dark / Adaptive 用に同梱。
- **セッション名の表示**: タブバーに文字は出せないので、色＋左パネルの行（セッション名・同色の下線）で。

- **同じセッションのタブを隣接させる**: セッション経由で開いたタブ（Recent Activity のクリック・`openFile`・`scripts/open_file.py`・そのセッションが書いたファイル）は、同セッションのタブの隣に置かれます。手でドラッグしたタブは以後動かしません。*Session Tabs — Regroup All Tabs by Session* でペイン全体を並べ直せます。
- **パネル**: セッション行に同色の下線と `① ⧉3`（バッジ・開いているタブ数）・`⊟2`（折りたたみ中のタブ数）を表示。行をクリックするとそのセッションの最新タブへ移動（折りたたみ中なら一括で開き直し）。
- **セッションメニュー**: タブ（またはパネルのセッション行）を右クリック → *Session Tabs*: 最新タブへ移動／タブのパスを一括コピー／ペインの先頭・末尾へまとめて移動／折りたたみ（閉じてウィンドウに記憶・行から開き直し）／一括で閉じる（未保存タブは残す）。コマンドパレットからは「アクティブなタブのセッション」に対して同じ操作ができます。

`scripts/open_file.py` は `$CLAUDE_CODE_SESSION_ID` を自動で付けます（`--session <id>` で上書き）。設定: `session_tabs.colors`・`palette`・`tint`・`badges`・`group`。
...
編集提案が Sublime 内に side-by-side diff タブとして開きます。提案ペインの ✓/✗ ボタン（コマンドパレットからも可。キーバインドは `Example.sublime-keymap` にコピペ用の見本を同梱）で Accept / Reject、または提案を手直ししてから Accept。あなたが判断するまで Claude は待機し、既定の権限設定ではレビューなしにディスクへ書き込まれることはありません。レビュー中は既定でウィンドウが 2 列になります。`diff_layout` を `"split"` にすると、そのファイルのあるペインだけを左右に割ります。最後のレビューを閉じると、元のレイアウトと各タブの位置が戻ります（グリッドや複数行のレイアウトも）。
