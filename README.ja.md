...
`scripts/open_file.py` は `$CLAUDE_CODE_SESSION_ID` を自動で付けます（`--session <id>` で上書き）。設定: `session_tabs.colors`・`tint`・`badges`・`group`。
