...
Claude セッションが開いた・書いたタブにはそのセッションが記録され、タブバーをセッション単位で**まとめて扱える**ようになります。Sublime はタブ 1 枚に色や印を付けられない（テーマの `settings` セレクタはグローバル設定しか見ない／ファイル付きタブへの `View.set_name` はファイルとの紐付けを外す。いずれも実機確認済みで、どちらも使っていません）ため、見分けは「並び」と「左パネル」で付けます。
...
- **パネルのバッジ**: セッション行に `① ⧉3`（バッジ・開いているタブ数）と `⊟2`（折りたたみ中のタブ数）を表示。行をクリックするとそのセッションの最新タブへ移動（折りたたみ中なら一括で開き直し）。
...
`scripts/open_file.py` は `$CLAUDE_CODE_SESSION_ID` を自動で付けます（`--session <id>` で上書き）。設定: `session_tabs.badges`・`session_tabs.group`。
