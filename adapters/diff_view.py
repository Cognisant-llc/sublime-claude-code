...
Two layouts (setting ``diff_layout``): ``"columns"`` swaps the window to a
two-column layout for the review; ``"split"`` splits only the pane that
holds the file (or the active pane) in half. Either way the window's layout
and every tab's position are saved when the first review opens and put back
when the last one closes — including grids and multi-row layouts, whose
lower panes Sublime would otherwise leave merged into the upper ones.
...
COLUMNS_LAYOUT = {"cols": [0.0, 0.5, 1.0], "rows": [0.0, 1.0],
...
# window id -> {"layout", "sheets": [(view_id, group, index)], "active": view_id}
# captured when the first review opens in that window, restored after the last
_window_state = {}
...
def diff_layout_mode():
    """``"columns"`` (default) or ``"split"``; anything else = columns."""
    mode = _settings().get("diff_layout", "columns")
    return "split" if mode == "split" else "columns"
...
def open_diff_ui(client_id, request_id, old_file_path, new_file_path,
    ...
    if diff_layout_mode() == "split":
        source = _find_view_group(window, target)
        if source is None:
            source = window.active_group()
        new_layout, right_group = L.split_right(window.layout(), source)
        window.set_layout(new_layout)
        left_group = source
    else:
        if window.layout() != COLUMNS_LAYOUT:
            window.set_layout(COLUMNS_LAYOUT)
...
def _remember_window(window):
    """Snapshot the layout, every tab's (group, index) and the active tab —
    once per window, when no review is open in it yet."""
    if window.id() in _window_state:
        return
    sheets = []
    for sheet in window.sheets():
        ...
        if group is not None and group >= 0:
            sheets.append((view.id(), group, index))
    active = window.active_view()
    _window_state[window.id()] = {
...
def _restore_window(window):
    st = _window_state.pop(window.id(), None)
    if st is None:
        return
    window.set_layout(st["layout"])
    by_id = {v.id(): v for v in window.views()}
    for vid, group, index in L.restore_order(st["sheets"], list(by_id),
                                              window.num_groups()):
        try:
            index = min(index, len(window.views_in_group(group)))
            window.set_view_index(by_id[vid], group, index)
        except Exception:  # noqa: BLE001 - best effort, never block teardown
            pass
    active = by_id.get(st["active"])
